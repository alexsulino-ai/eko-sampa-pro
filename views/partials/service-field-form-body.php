...
<div class="space-y-3">
    <div class="grid gap-2 sm:grid-cols-2">
        <label class="block text-xs font-medium text-slate-600"><?php echo esc_html__('Label', 'eko-sampa'); ?> *
            <input class="mt-1 w-full rounded-lg border border-slate-200 px-2 py-1.5 text-sm focus:border-indigo-300 focus:outline-none focus:ring-2 focus:ring-indigo-500/20" type="text" x-model="state.fieldForm.label" />
        </label>
        <label class="block text-xs font-medium text-slate-600"><?php echo esc_html__('Slug', 'eko-sampa'); ?> *
            <input class="mt-1 w-full rounded-lg border border-slate-200 px-2 py-1.5 text-sm font-mono focus:border-indigo-300 focus:outline-none focus:ring-2 focus:ring-indigo-500/20" type="text" x-model="state.fieldForm.slug" />
        </label>
        <label class="block text-xs font-medium text-slate-600"><?php echo esc_html__('Type', 'eko-sampa'); ?>
            <select class="mt-1 w-full rounded-lg border border-slate-200 px-2 py-1.5 text-sm focus:border-indigo-300 focus:outline-none focus:ring-2 focus:ring-indigo-500/20" x-model="state.fieldForm.type" @change="if (state.fieldForm.type === 'select' && (state.fieldForm.options_json == null || state.fieldForm.options_json === '')) { state.fieldForm.options_json = '[]'; }">
        ...
        <label class="flex items-center gap-2 self-end pb-1 text-sm text-slate-700">
            <input type="checkbox" x-model="state.fieldForm.required" :true-value="1" :false-value="0" />
    ...
    <div class="grid gap-2 sm:grid-cols-2">
        <label class="block text-xs font-medium text-slate-600 sm:col-span-2"><?php echo esc_html__('Default value', 'eko-sampa'); ?>
            <input class="mt-1 w-full rounded-lg border border-slate-200 px-2 py-1.5 text-sm" type="text" x-model="state.fieldForm.default_value" />
        </label>
        <label class="block text-xs font-medium text-slate-600 sm:col-span-2"><?php echo esc_html__('Placeholder (form hint)', 'eko-sampa'); ?>
            <input class="mt-1 w-full rounded-lg border border-slate-200 px-2 py-1.5 text-sm" type="text" x-model="state.fieldForm.placeholder" />
        </label>
    </div>

    <label class="block text-xs font-medium text-slate-600" x-show="state.fieldForm.type === 'select'">
        <?php echo esc_html__('Select options (JSON array)', 'eko-sampa'); ?>
        <textarea class="mt-1 w-full rounded-lg border border-slate-200 px-2 py-1.5 font-mono text-xs" rows="3" placeholder='["Option A","Option B"]' x-model="state.fieldForm.options_json"></textarea>
    </label>
    ...
    <div class="rounded-lg border border-slate-100 bg-slate-50/80 p-3">
        ...
        <div class="mt-3 space-y-2" x-show="state.fieldForm.type === 'text' || state.fieldForm.type === 'textarea'">
            <div class="grid gap-2 sm:grid-cols-2">
                <label class="block text-[11px] font-medium text-slate-600"><?php echo esc_html__('Min length', 'eko-sampa'); ?>
                    <input class="mt-0.5 w-full rounded-lg border border-slate-200 px-2 py-1 text-sm" type="number" min="0" step="1" x-model="state.fieldForm.validation.minLength" />
                </label>
                <label class="block text-[11px] font-medium text-slate-600"><?php echo esc_html__('Max length', 'eko-sampa'); ?>
                    <input class="mt-0.5 w-full rounded-lg border border-slate-200 px-2 py-1 text-sm" type="number" min="0" step="1" x-model="state.fieldForm.validation.maxLength" />
                </label>
            </div>
            <label class="block text-[11px] font-medium text-slate-600"><?php echo esc_html__('Pattern (regex)', 'eko-sampa'); ?>
                <input class="mt-0.5 w-full rounded-lg border border-slate-200 px-2 py-1 font-mono text-xs" type="text" x-model="state.fieldForm.validation.pattern" placeholder="<?php echo esc_attr__('e.g. ^[0-9]+$', 'eko-sampa'); ?>" />
            </label>
        </div>
        <div class="mt-3 space-y-2" x-show="state.fieldForm.type === 'number'">
            <div class="grid gap-2 sm:grid-cols-2">
                <label class="block text-[11px] font-medium text-slate-600"><?php echo esc_html__('Minimum', 'eko-sampa'); ?>
                    <input class="mt-0.5 w-full rounded-lg border border-slate-200 px-2 py-1 text-sm" type="text" inputmode="decimal" x-model="state.fieldForm.validation.minimum" />
                </label>
                <label class="block text-[11px] font-medium text-slate-600"><?php echo esc_html__('Maximum', 'eko-sampa'); ?>
                    <input class="mt-0.5 w-full rounded-lg border border-slate-200 px-2 py-1 text-sm" type="text" inputmode="decimal" x-model="state.fieldForm.validation.maximum" />
                </label>
            </div>
        </div>
        <div class="mt-3 space-y-2" x-show="state.fieldForm.type === 'date'">
            <label class="block text-[11px] font-medium text-slate-600"><?php echo esc_html__('Format hint (optional)', 'eko-sampa'); ?>
                <input class="mt-0.5 w-full rounded-lg border border-slate-200 px-2 py-1 text-sm" type="text" x-model="state.fieldForm.validation.format" placeholder="<?php echo esc_attr__('e.g. date', 'eko-sampa'); ?>" />
            </label>
        </div>
